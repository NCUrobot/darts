...
int32_t referee_reset_flag;
int32_t referee_run_flag;
uint32_t referee_student_run_flag;
/* 内部函数原型声明 ----------------------------------------------------------*/
  ...
  if (__HAL_UART_GET_FLAG(&RD_huart, UART_FLAG_IDLE))
	{
      ...
      //数据异常则重启
      if(off_line.device_offline_flag & 0x20)
      {
        Referee_Restart(REFEREE_RX_BUFFER_SIZE);
        referee_reset_flag++;
      }	
			/* 任务通知 */
...
{
		//disable UART
	__HAL_UART_DISABLE(&RD_huart);
	//disable DMA
	__HAL_DMA_DISABLE(RD_huart.hdmarx);
	//reset set_data_lenght
	RD_huart.hdmarx->Instance->NDTR = dma_buf_num;
	__HAL_UART_CLEAR_IDLEFLAG(&RD_huart);
	__HAL_DMA_CLEAR_FLAG(RD_huart.hdmarx,DMA_FLAG_TCIF1_5);
	//ensable UART
	__HAL_UART_ENABLE(&RD_huart);
	//ensable DMA
	__HAL_DMA_ENABLE(RD_huart.hdmarx);
	RefereeDate_Receive_USART_Init();
}
...
void Robot_Interactive_Date(uint8_t data[], uint16_t robot_interactive_id, uint16_t receive_robot_type)
{
  ...
  //设置发送的数据
  referee_send.robot_interactive_data.data[0] = data[0];//可继续添加,不能超过10个
  //将机器人交互数据复制到待发送数组中
